...
class kln_max(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X_test):
        ...
        for i, test_sample in enumerate(X_test):
            ...
            for c in all_labels:
                ...
                max_dist = np.max(np.partition([anydistance(test_sample, x, self.metric) for x in self.X_train_[self.y_train_ == c]], self.e)[self.e])
    ...
    def compute_score(self, max_dist , c):

        return np.math.exp(-np.abs(max_dist - self.EKNN_Statistics_[c][eknn_table.statTyp.meanofmax]) / self.EKNN_Statistics_[c][eknn_table.statTyp.stdofmax])
